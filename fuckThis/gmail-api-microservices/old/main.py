...
def process_new_user_registration(email: str, name: str, google_user_id: str):
    ...
    try:
        ...
        # Register user with email sync service
        try:
            sync_response = requests.post(
                f"{EMAIL_SYNC_SERVICE_URL}/register/user/{user_id}",
                timeout=5  # Set a reasonable timeout
            )
            
            if sync_response.status_code == 200:
                print(f"✅ User {email} registered with email sync service")
                print(f"   Queue ID: {sync_response.json().get('queue_id')}")
            else:
                print(f"⚠️ Failed to register with sync service: {sync_response.text}")
        except requests.RequestException as e:
            print(f"⚠️ Error contacting email sync service: {e}")
...
async def sync_latest_emails_until_overlap(user_id: str):
    """
    Synchronizes all new emails since the user's last session.
    Fetches emails until finding one that already exists in the database.
    
    Args:
        user_id: User's database ID
    
    Returns:
        Total number of newly synced emails
    """
    print(f"🔄 Starting login email sync for user {user_id}")
    
    creds = credential_manager.load_credentials(user_id)
    if not creds or not creds.valid:
        print(f"⚠️ Cannot sync emails: invalid credentials for user {user_id}")
        return 0
        ...
        for batch in range(max_batches):
            ...
            for message in messages:
                ...
                try:
                    ...
                    # Set category to None for all emails as requested
                    category = None
            ...
            # Save this batch of emails
            if email_list:
                saved_count = email_manager.save_emails(user_id, email_list)
                total_saved += saved_count
                print(f"📥 Batch {batch+1}: Saved {saved_count} new emails")
            
            # If we found an overlap or no more pages, we're done
            if found_overlap or not next_page_token:
                break
        
        print(f"✅ Login sync completed: saved {total_saved} new emails")
        return total_saved
        
    except Exception as e:
        print(f"❌ Error syncing emails on login: {str(e)}")
        return 0